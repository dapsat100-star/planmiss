...
def generate_5km_grid_covering(aoi_geojson: dict) -> GridResult:
    ...
    for t in tiles:
        ...
        total_area += t.area / 1e6  # m² -> km² (área no CRS projetado seria melhor)
...
with col_left:
    ...
    if st.button("Confirmar AOI", type="primary"):
        ...
        elif left_map and left_map.get("all_drawings") and left_map["all_drawings"].get("features"):
            # Une todos os polígonos
            try:
                geoms = [shape(f["geometry"]) for f in left_map["all_drawings"]["features"] if f["geometry"]["type"] in ("Polygon","MultiPolygon","GeometryCollection")]
                if geoms:
                    merged = unary_union(geoms).buffer(0)
                    feat = {"type":"Feature","properties":{},"geometry":mapping(merged)}
            except Exception:
                st.error("Não consegui ler os desenhos. Tente novamente com um único polígono.")
        if not feat:
