# -*- coding: utf-8 -*-
# app.py — GHGSat 5×5 km Planner (v9 — TLE offline no repositório)
# - Apenas GHGSat (footprint 5 km × 5 km, 1 tile)
# - NÃO baixa TLE da internet. Lê de um arquivo local do repositório: ./tle/ghgsat.tle
# - Você atualiza o arquivo manualmente (commit/push no GitHub) sempre que quiser.
#
# Fluxo:
# 1) Desenhe a AOI e confirme.
# 2) Escolha o satélite (GHGSAT-C1..C12 ou NORAD) e uma data/hora UTC.
# 3) Clique em Planejar: o app pega o TLE do arquivo local, calcula o azimute e desenha o footprint 5×5 km.
...
def _bearing_to_ccw_deg(bearing_deg: float) -> float:
    """Converte azimute de bússola (0=N, horário) para ângulo geométrico do Shapely (0=E, anti-horário)."""
...
def load_tle_file(path: str):
    """Lê o arquivo TLE e retorna uma lista de blocos [ (name, l1, l2) ]."""
...
def index_tles(blocks):
    """Cria dois índices: por NOME (linha 0) e por NORAD (catnr da linha 1)."""
    by_name = {}
    by_norad = {}
...
def get_tle_from_cache(name_or_catnr: str):
    """Resolve um TLE a partir do arquivo local, por nome (GHGSAT-Cx) ou NORAD."""
    blocks = load_tle_file(TLE_PATH)
    by_name, by_norad = index_tles(blocks)
    q = name_or_catnr.strip().upper()
    q1 = q.replace('-', ' ')
    # tenta por nome
    if q in by_name:
        return by_name[q]
    ...
    if q.isdigit() and q in by_norad:
        return by_norad[q]
    # tenta achar por substring (ex.: "C10")
    for k in by_name.keys():
        if q in k or q1 in k:
            return by_name[k]
    raise KeyError(f"Não achei TLE no cache local para: {name_or_catnr}")

def compute_track_azimuth_from_tle_block(tle_block, when_utc: datetime) -> float:
    """Calcula bearing (bússola) a partir de um bloco (name,l1,l2) usando Skyfield."""
...
def tle_file_info(path: str):
    """Retorna tamanho e data de modificação para avisar desatualização."""
    try:
        st = os.stat(path)
...
# Info do arquivo TLE
size, mtime = tle_file_info(TLE_PATH)
with st.sidebar:
    st.header("📁 Cache TLE local")
    if size is None:
        st.error("Arquivo não encontrado: ./tle/ghgsat.tle")
    else:
        st.success(f"Arquivo encontrado: {TLE_PATH}")
        st.write(f"Tamanho: {size} bytes")
        st.write(f"Modificado em (UTC): {mtime.strftime('%Y-%m-%d %H:%M:%S')}")
        age_days = (datetime.now(timezone.utc) - mtime).days if mtime else 999
        if age_days >= 3:
            st.warning("⚠️ TLE com 3+ dias — considere atualizar o arquivo.")
...
with col_right:
    ...
    if plan:
        if not st.session_state['aoi']:
            ...
            # Projeta AOI e pega centróide em metros
            ...
            if mode == "Do arquivo TLE local":
                ...
                if tle_manual.strip():
                    try:
                        # parse simples: nome opcional + 2 linhas
