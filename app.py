...
def _bearing_to_ccw_deg(bearing_deg: float) -> float:
    """Converte azimute (0°=N, sentido horário) para ângulo CCW usado pelo Shapely (0°=E)."""
    # 0=N -> 90, 90=E -> 0, 180=S -> -90 (270), 270=W -> -180 (180)
    ang = 90.0 - bearing_deg
    # Normaliza para [-180, 180] para evitar giros exagerados
...
if 'az_is_compass' not in st.session_state: st.session_state['az_is_compass'] = False
...
with col_right:
    ...
    if plan:
        if not st.session_state['aoi']:
            ...
            if az is None and mode == "Azimute manual (graus)":
                st.session_state['az_is_compass'] = False
                az = float(az_manual)
            if az is None:
            ...
            # Footprint 5x5
            rot_deg = _bearing_to_ccw_deg(az) if st.session_state.get('az_is_compass') else az
    ...
    if st.session_state['foot']:
        ...
        st.write(f"Azimute: {st.session_state['az']:.2f}° " + ("(bússola)" if st.session_state.get('az_is_compass') else "(geométrico CCW)"))
